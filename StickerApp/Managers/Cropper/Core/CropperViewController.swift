...
open class CropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
    ...
    open lazy var topBar: UIView = {
        ...
        topBar.backButton.addTarget(self, action: #selector(backButtonPressed(_:)), for: .touchUpInside)
    ...
    func confirmButtonPressed(_: UIButton) {
        let state = saveState()
          if let image = self.originalImage.cropped(withCropperState: state) {
            let vc = EraseViewController(contentImg: image)
    ...
    @objc
    func backButtonPressed(_: UIButton) {
