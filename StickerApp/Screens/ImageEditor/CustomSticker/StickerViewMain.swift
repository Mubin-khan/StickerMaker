...
@objc public protocol StickerViewDelegate {
    @objc func stickerViewDidBeginMoving(_ stickerView: StickerViewMain)
    @objc func stickerViewDidChangeMoving(_ stickerView: StickerViewMain)
    @objc func stickerViewDidEndMoving(_ stickerView: StickerViewMain)
    @objc func stickerViewDidBeginRotating(_ stickerView: StickerViewMain)
    @objc func stickerViewDidChangeRotating(_ stickerView: StickerViewMain)
    @objc func stickerViewDidEndRotating(_ stickerView: StickerViewMain)
    @objc func stickerViewDidClose(_ stickerView: StickerViewMain)
    @objc func stickerViewDidTap(_ stickerView: StickerViewMain)
    @objc func stickerViewDidFlipped()
    @objc func stickerSliderHideShow(flag : Bool)
}

public class StickerViewMain: UIView {
    
    public var startTime : Double = .zero
    public var endTime : Double = .zero
    ...
    public var showEditingHandlers:Bool = true {
        didSet {
            ...
            delegate?.stickerSliderHideShow(flag: showEditingHandlers)
        }
    }
    ...
    func handleMoveGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            let newPoint : CGPoint = CGPoint(x: self.beginningCenter.x + (touchLocation.x - self.beginningPoint.x), y: self.beginningCenter.y + (touchLocation.y - self.beginningPoint.y))
        ...
        case .ended:
            ...
            if let superView = self.superview {
                let yT = (self.frame.width / 2) + newPoint.y - 30
                let yB = superView.bounds.height + (self.frame.width / 2) - 30
                
                let xL = (self.frame.width / 2) + newPoint.x - 30
                let xR = superView.bounds.width + (self.frame.width / 2) - 30
                
                if yT < 0 && xL < 0 {
                    self.center = CGPoint(x: 40 - (self.frame.width / 2), y: 40 - (self.frame.width / 2))
                    return
                }
                if yT < 0 && newPoint.x > xR {
                    self.center = CGPoint(x: xR, y: 40 - (self.frame.width / 2))
                    return
                }
                if xL < 0 && newPoint.y > yB {
                    self.center = CGPoint(x: 40 - (self.frame.width / 2), y: yB)
                    return
                }
                if newPoint.y > yB && newPoint.x > xR {
                    self.center = CGPoint(x: xR, y: yB)
                    return
                }
                
                if yT < 0 {
                    self.center = CGPoint(x: newPoint.x, y: 30 - (self.frame.width / 2) )
                    return
                }else if newPoint.y > yB {
                    self.center = CGPoint(x: newPoint.x, y: yB)
                    return
                }else if xL < 0 {
                    self.center = CGPoint(x: 30 - (self.frame.width / 2), y: newPoint.y)
                    return
                }else if newPoint.x > xR {
                    self.center = CGPoint(x: xR, y: newPoint.y)
                    return
                }
            }
    ...
    func handleRotateGesture(_ recognizer: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            if let superView = self.superview {
                if scaledBounds.width > superView.bounds.width || scaledBounds.height > superView.bounds.height {
                    return
                }
            }
    ...
    func handleFlipGesture(_ recognizer: UITapGestureRecognizer) {
        UIView.animate(withDuration: 0.3) {
            ...
            if let delegate = self.delegate {
                delegate.stickerViewDidFlipped()
            }
        }
    }
...
extension StickerViewMain: UIGestureRecognizerDelegate {
